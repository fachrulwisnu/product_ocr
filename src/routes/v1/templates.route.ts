...
let inMemoryTemplates: ReceiptTemplate[] = [
  {
    ...
    keywords: ['REMAINING NOTES', 'WITHDRAW COUNTER', 'DEPOSIT COUNTER', 'STORED REJECT', 'CASSETTE', 'REMAINING'],
...
// Auto-seed or sync Supabase receipt_templates_ocr table
async function syncSupabaseReceiptTemplates() {
  if (!isSupabaseConfigured()) return;
  try {
    const { data } = await supabase.from('receipt_templates_ocr').select('*');
    if (!data || data.length === 0) {
      await supabase.from('receipt_templates_ocr').insert(inMemoryTemplates.map(t => ({
        template_name: t.template_name,
        keywords: t.keywords,
        schema_rule: t.schema_rule
      })));
    } else {
      // Ensure Cassette Audit keywords exist in DB
      const cassetteTmpl = data.find((t: any) => t.template_name?.toLowerCase().includes('cassette') || t.template_name?.toLowerCase().includes('audit'));
      if (cassetteTmpl) {
        await supabase.from('receipt_templates_ocr').update({
          keywords: ['REMAINING NOTES', 'WITHDRAW COUNTER', 'DEPOSIT COUNTER', 'STORED REJECT', 'CASSETTE', 'REMAINING']
        }).eq('id', cassetteTmpl.id);
      } else {
        await supabase.from('receipt_templates_ocr').insert([{
          template_name: 'Cassette Audit',
          keywords: ['REMAINING NOTES', 'WITHDRAW COUNTER', 'DEPOSIT COUNTER', 'STORED REJECT', 'CASSETTE', 'REMAINING'],
          schema_rule: 'Format as JSON with keys: BANK_NAME, ATM_ID, CASSETTE_1_DISPENSED, CASSETTE_2_DISPENSED, TOTAL_AUDIT_BALANCE, AUDIT_DATE, AUDIT_TIME, OPERATOR_ID, REMAINING_NOTES, WITHDRAW_COUNTER, DEPOSIT_COUNTER, STORED_REJECT.'
        }]);
    ...
    console.warn("Could not sync receipt_templates_ocr in Supabase:", err);
  }
}
syncSupabaseReceiptTemplates();
